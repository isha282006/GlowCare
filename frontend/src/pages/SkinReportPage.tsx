...
import { Save, ArrowRight, Check, BookOpen, Sparkles, Heart } from 'lucide-react';
...
const SkinReportPage: React.FC = () => {
  ...
  // Report comes from the backend (user.skinReport) — generated by the recommendation API
  const report = user?.skinReport || {
    assessment: { skinType: 'Normal', primaryConcerns: [], hydration: 'Medium', oilLevel: 'Moderate', skinSensitivity: 'Low', skinScore: 80, confidence: 'Questionnaire-based assessment', date: new Date().toLocaleDateString() },
    routine: { morning: [], night: [] },
    recommendations: { products: [], ingredients: [], tips: [], weeklyCare: [], healthyHabits: [] },
    disclaimer: ''
  };
      ...
      <div className="max-w-4xl mx-auto space-y-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            {recommendations.products && recommendations.products.length > 0 && (
              <div className="glass-card p-6 space-y-5 text-left">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {recommendations.products.map((prod: any, i: number) => (
            ...
            {/* Healthy Habits */}
            {recommendations.healthyHabits && recommendations.healthyHabits.length > 0 && (
              <div className="glass-card p-6 space-y-4 text-left">
                <h3 className="font-extrabold text-sm flex items-center gap-2 text-gray-700">
                  <Heart className="text-red-400" size={16} /> Healthy Habits for Better Skin
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  {recommendations.healthyHabits.map((habit: string, i: number) => (
                    <div key={i} className="p-3 rounded-xl border bg-green-50/30 border-green-100/40 flex items-start gap-2 text-xs font-semibold text-gray-600">
                      <span className="flex-shrink-0">{habit.slice(0, 2)}</span>
                      <span className="leading-relaxed">{habit.slice(2).trim()}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {report.warnings && report.warnings.length > 0 && (
              <div className="glass-card p-6 space-y-4 text-left border-amber-200/50">
                <h3 className="font-extrabold text-sm flex items-center gap-2 text-amber-700">
                  ⚠️ Important Warnings
                </h3>
                <div className="grid grid-cols-1 gap-2">
                  {report.warnings.map((warning: string, i: number) => (
                    <div key={i} className="p-3 rounded-xl bg-amber-50/40 border border-amber-100/50 text-xs font-semibold text-amber-800">
                      {warning}
                    </div>
                  ))}
                </div>
