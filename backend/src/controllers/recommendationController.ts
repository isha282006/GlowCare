...
import RecommendedProduct, { IRecommendedProduct } from '../models/RecommendedProduct';
...
// ═══════════════════════════════════════════════════
function generateLifestyleTips(skinType: string, concerns: string[], lifestyle: any): string[] {
  const tips: string[] = [];
...
function calculateSkinScore(concerns: string[], lifestyle: any): number {
  ...
  if (lifestyle?.waterIntake?.includes('< 1L')) score -= 3;
  if (lifestyle?.sunscreenUsage === 'Rarely' || lifestyle?.sunscreenUsage === 'Never') score -= 4;
...
function buildRoutine(
  ...
  const findBest = (category: string): IRecommendedProduct | undefined => {
...
export const generateRecommendation = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const matchedProducts = allProducts
      .filter(p =>
        p.skinTypes.includes(skinType) &&
        p.skinConcerns.some((c: string) => concerns.includes(c))
      )
      .map(p => ({
        name: p.name,
        brand: p.brand,
    ...
    if (skinType === 'Oily') oilLevel = 'High';
    else if (skinType === 'Dry') oilLevel = 'Low';

    let sensitivity: 'High' | 'Medium' | 'Low' = 'Low';
    if (skinType === 'Sensitive') sensitivity = 'High';
    else if (skinType === 'Combination') sensitivity = 'Medium';
...
function generateWarnings(skinType: string, concerns: string[]): string[] {
...
export const getRecommendedProducts = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const products = await RecommendedProduct.find({}).sort({ category: 1, brand: 1 });
    res.status(200).json({
      success: true,
      data: products,
...
export const getMatchedProducts = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (concerns) {
      const concernList = (concerns as string).split(',');
      filter.skinConcerns = { $in: concernList };
    }

    const products = await RecommendedProduct.find(filter).sort({ category: 1 });
    res.status(200).json({
      success: true,
      data: products,
      count: products.length,
