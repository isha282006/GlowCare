...
exports.checkAchievements = exports.getAchievements = void 0;
...
const getAchievements = async (req, res) => {
    try {
        let achievements = await Achievement_1.default.find({ user: req.user._id });
        ...
        if (achievements.length === 0) {
            ...
            achievements = await Achievement_1.default.insertMany(newAchievements);
        }
...
const checkAchievements = async (req, res) => {
    try {
        ...
        let achievements = await Achievement_1.default.find({ user: userId });
        if (achievements.length === 0) {
            const newAchievements = ACHIEVEMENT_DEFINITIONS.map((def) => ({
                user: userId,
                ...def,
        ...
        const routineHistoryCount = await RoutineHistory_1.default.countDocuments({
            user: userId,
            completionPercentage: { $gt: 0 },
        });
        const morningRoutines = await RoutineHistory_1.default.countDocuments({
            user: userId,
            routineType: 'morning',
            completionPercentage: { $gte: 100 },
        });
        const nightRoutines = await RoutineHistory_1.default.countDocuments({
            user: userId,
            routineType: 'night',
            completionPercentage: { $gte: 100 },
        });
        const journalCount = await JournalEntry_1.default.countDocuments({ user: userId });
        const productCount = await Product_1.default.countDocuments({ user: userId });
        // Calculate streaks
        const history = await RoutineHistory_1.default.find({ user: userId }).sort({ date: -1 });
        const uniqueDates = [...new Set(history.filter((h) => h.completionPercentage > 0)
                .map((h) => h.date.toISOString().split('T')[0]))].sort().reverse();
        let currentStreak = 0;
        ...
        // Hydration days
        const hydrationDays = await JournalEntry_1.default.countDocuments({
            user: userId,
            waterIntake: { $gte: 8 },
        });
        // Sleep days
        const sleepDays = await JournalEntry_1.default.countDocuments({
            user: userId,
            sleepHours: { $gte: 7 },
        });
        // Both routines days
        const bothRoutineDays = await RoutineHistory_1.default.aggregate([
        ...
        for (const achievement of achievements) {
            ...
            await Achievement_1.default.findByIdAndUpdate(achievement._id, {
                progress: Math.min(progress, achievement.target),
                isUnlocked,
                unlockedAt: isUnlocked && !achievement.isUnlocked ? new Date() : achievement.unlockedAt,
            });
        }
        const updated = await Achievement_1.default.find({ user: userId });
